...
public class DeleteBookCategory {

	@Autowired
	CategoryService cateBook;

	@PostMapping("/deleteCategory")
	public ResponseSend deleteBookCategory(@RequestBody CategoryModel catData) {
		ResponseSend response = new ResponseSend();
		try {
			cateBook.delete(catData);
			response.setStatusCode(ConstantInfo.DELETE_BOOK_CATEGORY_SAVE_MSG_CODE);
			response.setStatusMessage(ConstantInfo.DELETE_BOOK_CATEGORY_MASSAGE_SAVE);
